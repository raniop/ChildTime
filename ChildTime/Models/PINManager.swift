...
final class PINManager {
    ...
    /// Erase the parent PIN from the Keychain. The Keychain survives an app
    /// delete/reinstall, so a full data-wipe must clear it explicitly — otherwise
    /// the old code lingers on the device.
    func deletePIN() {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: service,
