...
struct SessionProgressHUD: View {
    ...
    var body: some View {
        HStack(spacing: compact ? 6 : 10) {
            if showsEarn {
                ...
                chip(
                    emoji: "🎮",
                    text: tr("הַיּוֹם \(earnedToday) דַּק׳"),
            ...
            chip(
                emoji: "🎁",
                text: wheelReady ? tr("גַּלְגַּל מוּכָן!") : tr("גַּלְגַּל בְּ-\(questionsUntilWheel)"),
            ...
            chip(
                emoji: "⭐",
                text: tr("רָמָה בְּ-\(questionsUntilLevel)"),
    ...
    private var rewardText: String {
        questionsUntilReward <= 0 ? tr("פְּרָס!") : tr("פְּרָס בְּ-\(questionsUntilReward)")
    }
