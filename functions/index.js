  ...
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#EFECFA;padding:24px 12px;">
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="width:100%;max-width:600px;background:#FFFFFF;border-radius:22px;overflow:hidden;box-shadow:0 10px 30px rgba(30,18,80,.10);">
        ...
        <!-- body -->
        <tr>
          <td style="padding:30px 26px 8px 26px;" ${ltr ? `dir="ltr" align="left"` : `dir="rtl" align="right"`}>
        ...
        <!-- signoff -->
        <tr>
          <td style="padding:0 26px 28px 26px;" ${ltr ? `dir="ltr" align="left"` : `dir="rtl" align="right"`}>
...
// Russian: the same suffix. Device kinds keep their Latin product names.
function deviceLabelRu(event) {
  const nm = (event.deviceName || "").trim();
  const generic = ["מכשיר", "Device", "Устройство", "iPhone", "iPad", "iPod touch"];
  if (nm && !generic.includes(nm)) return ` · ${nm}`;
...
// Parent-facing Russian for the same events (formal «вы»). Gender comes from the
// event, as in Hebrew; returns null for exactly the types the Hebrew skips.
function liveMessageRu(event) {
  const f = event.gender === "girl";
  const name = event.childName || (f ? "Ваша дочь" : "Ваш ребёнок");
  const g = (male, female) => (f ? female : male);
  const dev = deviceLabelRu(event);
  switch (event.type) {
    case "sessionStart": return { title: g("Начал играть 📱", "Начала играть 📱"), body: `${name} ${g("начал", "начала")} играть и учиться${dev}.` };
    case "sessionEnd": {
      const q = Number(event.questions) || 0;
      const acc = Number(event.accuracy) || 0;
      const mins = Number(event.minutes) || 0;
      const stars = Number(event.stars) || 0;
      const parts = [];
      if (q > 0) parts.push(`${q} вопр.`);
      if (q > 0) parts.push(`${acc}% верно`);
      if (mins > 0) parts.push(`${mins} мин. заработано`);
      if (stars > 0) parts.push(`${stars} ⭐`);
      const summary = parts.length ? parts.join(" · ") : g("закончил занятие", "закончила занятие");
      return { title: g("Закончил играть ✅", "Закончила играть ✅"), body: `${name}${dev}: ${summary}` };
    }
    case "screenTimeStart": {
      const mins = Number(event.minutes) || 0;
      const tail = mins > 0 ? ` (${mins} мин.)` : "";
      return { title: g("Открыл игровое время 🎮", "Открыла игровое время 🎮"), body: `${name} ${g("открыл", "открыла")} игровое время${tail}${dev}.` };
    }
    case "screenTimeEnd": {
      const mins = Number(event.minutes) || 0;
      const tail = mins > 0 ? ` · осталось мин.: ${mins}` : "";
      return { title: g("Закончил игровое время ⏹️", "Закончила игровое время ⏹️"), body: `${name} ${g("закончил", "закончила")} игровое время${dev}${tail}.` };
    }
    case "screenTimeMoved": {
      const kindLabel = (k) => (k === "ipad" ? "iPad" : (k === "iphone" ? "iPhone" : "другое устройство"));
      const from = kindLabel(event.fromKind);
      const to = kindLabel(event.deviceKind);
      return { title: g("Перенёс игровое время 🔄", "Перенесла игровое время 🔄"),
               body: `${name} ${g("перенёс", "перенесла")} игровое время: ${from} → ${to}. Устройство ${from} заблокировано 🔒` };
    }
...
function premiumRequestMessage(after, lang) {
  ...
  if (lang === "ru") {
    const name = after.name || "Ваш ребёнок";
    const topic = TOPIC_RU[after.premiumRequestedTopic || ""];
    return topic
      ? { title: `${name} хочет открыть мир: ${topic}`,
...
function packRequestMessage(after, lang) {
  ...
  if (lang === "ru") {
    ...
    const p = PACK_META[packID];
    return p
      ? { title: `${p.emoji} ${name} просит набор «${p.nameRu}»`,
          body: `${name} хочет узнать больше про это: ${p.subjectRu}. Это разовое дополнение — откройте его со своего телефона.` }
...
function liveGameInviteMessage(kind, rawHostName, lang) {
  ...
  if (lang === "ru") {
    const hostName = rawHostName || "Друг";
    return kind === "nudge"
...
function timeTransferMessage(kind, after, lang) {
  ...
  if (lang === "ru") {
    if (kind === "seller") {
      return { title: "🛒 Просьба купить время", body: `${after.toName} хочет купить у тебя минут: ${after.minutes} за ${after.diamondPrice} 💎` };
    }
    const body = (after.diamondPrice > 0)
      ? `Покупает ${after.toName}, продаёт ${after.fromName} — минут: ${after.minutes}. Нужно ваше подтверждение`
...
function childLinkMessage(kind, after, lang) {
  ...
  if (lang === "ru") {
    return kind === "request"
      ? { title: "Запрос на присоединение к семье 👨‍👩‍👧",
...
function waitlistWelcomeEmail(w, lang) {
  ...
  if (lang === "ru") {
    ...
    const bullets = [
      ...
      { emoji: "💝", title: "Две недели Tofy+ в подарок", text: "Когда ребёнок по-настоящему начнёт учиться, мы откроем все миры на две недели. Без банковской карты." },
    ];
...
function requireAdmin(request) {
  ...
  if (!email || !verified || !ADMIN_EMAILS.map((e) => e.toLowerCase()).includes(email)) {
    throw new HttpsError("permission-denied", "Not an authorized admin.");
  }
  return email;
}
  ...
  hhSnap.forEach((h) => {
    ...
    if (wantChildren) {
      for (const k of matching) {
        ...
        if (!toks.length) continue;
        childrenN += 1;
        (devsByKid[k.id] || []).forEach((d) => { childTokens.add(d.fcmToken); owners[d.fcmToken] = { householdID: h.id, family, kind: "child", who: k.name || "ילד", deviceDocID: d.docID }; });
      }
    }
  });
...
function campaignCopy(c, forChild, lang) {
  ...
  if (lang === "ru") {
    ...
    const title = forChild
      ? (t(c.childTitleRu) || t(c.titleRu) || t(c.childTitleEn) || t(c.titleEn))
      : (t(c.titleRu) || t(c.titleEn));
    const body = forChild
      ? (t(c.childBodyRu) || t(c.bodyRu) || t(c.childBodyEn) || t(c.bodyEn))
      : (t(c.bodyRu) || t(c.bodyEn));
    const heBody = forChild ? (c.childBody || c.body) : c.body;
    if (!title || (!body && heBody)) return null;
    return { title, body };
  }
...
async function pruneDeadTokens(results, owners) {
  ...
  for (const r of dead) {
    ...
    if (!o) continue;
    try {
      if (o.uid) await db.collection("parents").doc(o.uid).update({ fcmTokens: admin.firestore.FieldValue.arrayRemove(r.token), childFcmTokens: admin.firestore.FieldValue.arrayRemove(r.token) });
      if (o.deviceDocID) await db.collection("childDevices").doc(o.deviceDocID).update({ fcmToken: admin.firestore.FieldValue.delete() });
    } catch (e) { /* best effort */ }
  }
  return dead.length;
}
...
function worldPassMessage(rawName, packID, at, lang, tz = DEFAULT_TZ) {
  ...
  if (lang === "ru") {
    const whose = rawName ? `у ребёнка ${rawName}` : "у вашего ребёнка";
...
exports.pruneChorePhotos = onSchedule(
  ...
  async () => {
    ...
    const snap = await db.collectionGroup("chores").where("markedDoneAt", "<=", cutoff).select("photoToken").get();
    let pruned = 0;
    for (const doc of snap.docs) {
      if (!(doc.data() || {}).photoToken) continue;
...
exports.adminSaveCampaign = onCall({ timeoutSeconds: 30, memory: "256MiB" }, async (request) => {
  ...
  const base = existing && existing.exists ? {} : { createdAt: Date.now(), createdBy: request.auth.uid,
    stats: Object.fromEntries(CAMPAIGN_STAT_KEYS.map((k) => [k, 0])) };
  await ref.set({ ...base, ...c, updatedAt: Date.now(), updatedBy: request.auth.uid }, { merge: true });
...
exports.adminDeleteCampaign = onCall({ timeoutSeconds: 30, memory: "256MiB" }, async (request) => {
  ...
  const id = String(request.data && request.data.id || "");
  if (!id) throw new HttpsError("invalid-argument", "id required");
  const ref = db.collection("campaigns").doc(id); const snap = await ref.get();
  if (snap.exists && snap.data().status === "sending") throw new HttpsError("failed-precondition", "sending");
...
exports.adminSetPackEnabled = onCall({ timeoutSeconds: 30, memory: "256MiB" }, async (request) => {
  ...
  await ref.set({ enabled, updatedAt: Date.now(), updatedBy: request.auth.uid,
    ...(enabled && !before.launchedAt ? { launchedAt: admin.firestore.FieldValue.serverTimestamp() } : {}) }, { merge: true });
...
async function computeJourney() {
  ...
  const il = (ms) => new Date(ms).toLocaleDateString("en-CA", { timeZone: "Asia/Jerusalem" });
  const todayKey = il(nowMs);
  const last14 = Array.from({ length: 14 }, (_, i) => il(nowMs - (13 - i) * 86400000));
  const [hhSnap, kidsSnap, parentsSnap, salesSnap, reportsSnap, feedbackSnap, packsSnap] = await Promise.all([
    db.collection("households").get(), db.collection("children").get(),
    db.collection("parents").get(), db.collection("packPurchases").get(),
    db.collection("questionReports").select("status").get().catch(() => ({ docs: [] })),
    db.collection("parentFeedback").select("status").get().catch(() => ({ docs: [] })),
    db.collection("packs").get().catch(() => ({ docs: [] })),
  ]);
  const openReports = (reportsSnap.docs || []).filter((d) => ((d.data() || {}).status || "open") === "open").length;
  const openFeedback = (feedbackSnap.docs || []).filter((d) => ((d.data() || {}).status || "open") === "open").length;
  const packsOn = (packsSnap.docs || []).filter((d) => (d.data() || {}).enabled).length;
  ...
  for (let i = 0; i < kids.length; i += CHUNK) {
    ...
    // Bounded: the app keeps writing one doc per day forever — 120 days is plenty for every panel.
    const snaps = await Promise.all(slice.map((k) => k.ref.collection("dailyStats").orderBy("date", "desc").limit(120).get().catch(() => ({ docs: [] }))));
  ...
  hhSnap.forEach((h) => {
    ...
    if (premium && !gift) {
      funnel.purchased += 1; premiumN += 1;
      const yearly = /yearly/.test(hh.premiumProduct || "") || (hh.premiumProduct ? false : daysLeft > 45) || premiumUntil > nowS + 400 * 86400;
  ...
  const journey = {
    ...
    overview: { activeFamilies30: active30, childrenToday, questionsToday, accuracyToday: questionsToday ? Math.round(100 * correctToday / questionsToday) : 0,
      ...
      openReports, openFeedback, packsOn, packsReady: Object.keys(PACK_META).length - packsOn, packSales: sales.length, packRevenue: Math.round(packRevenue),
    ...
    states, funnel, households,
    // Compact per-child copy (the 1 MB doc limit): the panels and the engine need only these.
    children: Object.fromEntries(Object.entries(perChild).map(([id, c]) => [id, { name: c.name, householdID: c.householdID, grade: c.grade, gender: c.gender,
      activeDays30: c.activeDays30, questions30: c.questions30, everQuestions: c.everQuestions, everActiveDays: c.everActiveDays, lastActive: c.lastActive,
      today: c.today, activated: c.activated, favorite: c.favorite, packs: c.packs }])),
  };
...
exports.adminSetConfig = onCall({ timeoutSeconds: 30, memory: "256MiB" }, async (request) => {
  ...
  for (const k of allowed) {
    ...
    const def = CONVERSION_DEFAULTS[k];
    if (typeof def === "number") { const n = Number(patch[k]); if (!Number.isFinite(n)) throw new HttpsError("invalid-argument", k); clean[k] = n; }
    else if (typeof def === "boolean") clean[k] = !!patch[k];
...
exports.adminSetPackMeta = onCall({ timeoutSeconds: 30, memory: "256MiB" }, async (request) => {
  ...
  await db.collection("packs").doc(id).set({ ...patch, updatedAt: Date.now(), updatedBy: request.auth.uid }, { merge: true });
  return { ok: true };
});
...
// The same, for Russian devices. {variables} are identical, and each keeps the
// "prefix · body" shape so the prefix is stripped the same way.
const CONVERSION_COPY_RU = {
  copyActivation: "🎁 Tofy+ в подарок — {שם} · Отвечено вопросов: {שאלות} — поэтому все миры открыты. Дней: {ימים}. Карта не нужна, подписка не продлевается. Заканчивается {תאריך}.",
  copyTwoDays: "⏰ Осталось 2 дня Tofy+ — {שם} · {עולם אהוב}: дней с занятиями за последний месяц: {חזרות}. Оставьте все миры открытыми с Tofy+.",
...
const TOPIC_LABEL_RU = { math: "🧮 Математика", english: "🇬🇧 Английский", hebrew: "✍️ Иврит", logic: "🧩 Логика", science: "🔬 Наука", history: "🏛️ История",
  geography: "🌍 География", money: "💰 Финансовая грамотность", reading: "📖 Понимание текста", soccer: "⚽ Футбол", dinosaurs: "🦖 Динозавры", space: "🚀 Космос", animals: "🐾 Животные",
  sea: "🌊 Море", gifted: "🧠 Подготовка для одарённых", food: "🍳 Еда", israel: "🏛️ Мой Израиль", music: "🎵 Музыка", body: "🧍 Тело человека", vehicles: "🚗 Транспорт", flags: "🌍 Флаги", tishrei: "🍎 Осенние праздники" };
const favLabelRu = (fav) => TOPIC_LABEL_RU[fav.topic] || capFirst(String(fav.topic || "")) || "Любимый мир";
...
function giftDayMessage(ctx, day, daysLeft, premiumUntil, lang) {
  ...
  if (lang === "ru") {
    const name = vars["שם"];
    const favLabel = fav ? favLabelRu(fav) : null;
    if (daysLeft <= 1) {
      const keep = fav ? `${favNameRu(fav)} и другие миры` : "все миры";
      return { title: `Подарок Tofy+ заканчивается сегодня — ${name}`, body: `${capFirst(name)} и дальше будет учиться и зарабатывать игровое время бесплатно, как обычно. Чтобы доступ сохранился (${keep}), продолжите с Tofy+.` };
    }
    if (daysLeft <= 2) {
      const tpl = fav ? CONVERSION_COPY_RU.copyTwoDays : CONVERSION_COPY_RU.copyTwoDaysNoFavorite;
      return { title: `⏰ Осталось 2 дня Tofy+ — ${name}`, body: capFirst(fill(tpl, vars).replace(/^⏰[^·]*·\s*/, "")) };
    }
    if (day <= 9) {
      return fav ? { title: `❤️ ${capFirst(name)} — кажется, любимый мир найден`,
                     body: `${favLabel} — это место, куда ${name} возвращается в Tofy чаще всего: дней: ${fav.days}, вопросов: ${fav.questions}.` } : null;
    }
...
exports.onHouseholdPremiumWritten = onDocumentWritten("households/{hid}", async (event) => {
  ...
  const now = Date.now() / 1000;
  const giftEnd = Number(after.giftUntil || 0) || (Number(after.giftStartedAt || 0) + Number(after.giftDays || 14) * 86400);
  const wasGift = after.premiumSource === "gift";
  // A gift grant/extension is written by the server (giftUntil) — skip it; a paid
  // entitlement reaches more than a week past whatever the gift covered.
  if (wasGift && a <= giftEnd + 60) return;
  if (a - Math.max(now, giftEnd) < 8 * 86400) return;
  if (after.premiumSource === "paid" && after.purchasedAt) { await event.data.after.ref.set({ renewedAt: now }, { merge: true }); return; }
...
function retentionNoticeEmail(lang) {
  ...
  if (lang === "ru") {
    const title = "Данные вашей семьи в Tofy будут удалены через 30 дней";
    const intro = "Больше полутора лет никто в вашей семье не пользовался Tofy. Чтобы защитить приватность детей, мы не храним информацию о семьях, которые перестали пользоваться приложением.";
    const bullets = [
      { emoji: "🗓️", title: "Что будет удалено", text: "Профили ваших детей, их прогресс и история занятий, домашние дела семьи и записи о подключённых устройствах — через 30 дней." },
      { emoji: "📱", title: "Хотите сохранить?", text: "Просто откройте Tofy на любом устройстве вашей семьи в ближайшие 30 дней, и всё останется точно так же." },
      { emoji: "✉️", title: "Есть вопросы?", text: "Напишите нам на ranioph@gmail.com — мы поможем." },
    ];
    const signoff = "Команда Tofy 🦁";
    const html = brandEmail({ title, intro, bullets, ctaText: "Наша политика конфиденциальности",
      ctaHref: "https://tofyapp.com/ru/privacy.html", signoff,
