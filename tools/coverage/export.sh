...
xcrun swiftc -Onone -parse-as-library -o $TMP/coverage \
  ...
  $M/Topic.swift $M/Question.swift $M/ContentLanguage.swift $M/EnglishContent/*.swift(N) $M/RussianContent/*.swift(N) $M/ArabicContent/*.swift(N) $M/RemoteQuestionBank.swift $ROOT/Shared/AppGroup.swift \
